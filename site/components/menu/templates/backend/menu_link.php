...
<form class="form-text" method="post" action="<?php echo url('action=cms/save_menu_link/post'); ?>" class="form-inline-elements">
  ...
  <div class="ctrlHolder">
    <fieldset class="fieldset-action">
      ...
      <ul>
        <li><label><input type="radio"<?php $data->link->action->eq(0, function(){ echo ' checked="checked"'; }); ?> value="0" name="link_action"> <?php __($names->component, 'Redirect'); ?></label></li>
        <li><label><input type="radio"<?php $data->link->action->eq(1, function(){ echo ' checked="checked"'; }); ?> value="1" name="link_action"> <?php __($names->component, 'Copy content'); ?></label></li>
      </ul>
    </fieldset>
  </div>
</form>
